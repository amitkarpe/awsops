...
- Issue #33 is FAST DEPLOY / Option A. Amit explicitly authorized the personal-LAB NEW public activation path using the existing retained host + proven Route53/Nginx pattern for `ops2/sec2`, while preserving OLD `ops/sec`. Small reversible acceptance-path changes do not require repeated approval: NEW TLS issuance/renewal, exact ACME DNS-01 TXT writes, NEW-only Nginx reload/service restart, and—only if needed—the narrowest Route53 TXT-write permission for the exact ACME name are authorized. Incremental recurring cost under USD 1/month is not a cost stop gate for this lane. No deletion, no new EC2/database/LB/NAT, no broad/admin IAM, and no OLD mutation. Bridge2 is only the G -> X control/message plane; its `aws=false`/shell flags do not constrain Codex's native AWS CLI/AWS MCP/operator-shell capabilities. Per Amit, X may execute the authorized personal-LAB Route53/TLS/Nginx/systemd work directly with its native tools instead of stopping at a copy-paste handoff.
