...
class CanaryBrowserTests(unittest.TestCase):
    ...
    def test_reject_browser_uses_bound_card_single_submit_and_archive(self):
        ...
        self.assertIn("select_agent_count", source)
        self.assertIn("select_agent_visible", source)
        self.assertIn("select_agent_enabled", source)
        self.assertIn("agent_name_matches", source)
        self.assertIn("agent_combobox_matches", source)
        self.assertIn("option_state", source)
        self.assertIn("center_hit", source)
        self.assertIn("select_completed", source)
        self.assertIn("option.boundingBox()", source)
        ...
        self.assertIn("await search.press('Enter')", source)
        self.assertIn("stage='agent_option_enter'", source)
        ...
        self.assertIn("const selectAgent=form.getByRole('button',{name:'Select Agent',exact:true})", source)
        self.assertIn("await selectAgent.click()", source)
        self.assertNotIn("selectAgent.count()", source)
        self.assertNotIn("stage='agent_selected_wait'", source)
        self.assertNotIn("AGENT_SELECTION_NOT_READY", source)
        self.assertIn("for(let i=0;i<120;i++)", source)
        self.assertIn("stage='agent_builder_open'", source)
        self.assertIn("stage='agent_builder_root'", source)
        self.assertIn("getByRole('button',{name:'Back to builder',exact:true})", source)
        self.assertIn("getByRole('combobox',{name:'Agent',exact:true})", source)
        self.assertIn("for(let i=0;i<120;i++)", source)
        self.assertIn("agentSelect.isVisible().catch(()=>false)", source)
        self.assertIn("backToBuilder.isVisible().catch(()=>false)", source)
        self.assertIn("AGENT_BUILDER_BACK_DISABLED", source)
        self.assertIn("AGENT_BUILDER_ROOT_REQUIRED", source)
        self.assertIn("stage='agent_combobox_click'", source)
        self.assertIn("AGENT_COMBOBOX_DISABLED", source)
        self.assertNotIn("agentSelect.evaluate(element=>element.click())", source)
        ...
        self.assertIn("AGENT_BUILDER_TRIGGER_REQUIRED", source)
        self.assertIn("AGENT_BUILDER_TRIGGER_DISABLED", source)
        self.assertIn("if(await button.getAttribute('aria-pressed')!=='true')await button.click()", source)
        self.assertNotIn("button.evaluate(element=>element.click())", source)
        self.assertNotIn("agent_builder_recover", source)
        self.assertIn("timeout:30000", source)
