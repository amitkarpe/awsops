...
class CanaryBrowserTests(unittest.TestCase):
    ...
    def test_reject_browser_uses_bound_card_single_submit_and_archive(self):
        ...
        self.assertIn("stage='agent_selected_wait'", source)
        self.assertIn("getByLabel('Agent name')", source)
        self.assertIn("selectedName.inputValue().catch(()=>null)===name", source)
        self.assertIn("AGENT_SELECTION_NOT_READY", source)
        self.assertIn("stage='agent_select_submit'", source)
        self.assertIn("if(await selectAgent.count()!==1)throw Error('AGENT_SELECT_REQUIRED')", source)
        self.assertNotIn("stage='agent_selected_wait'", source)
        self.assertNotIn("AGENT_SELECTION_NOT_READY", source)
        self.assertIn("AGENT_SELECT_REQUIRED", source)
