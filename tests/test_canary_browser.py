...
class CanaryBrowserTests(unittest.TestCase):
    ...
    def test_reject_browser_uses_bound_card_single_submit_and_archive(self):
        ...
        self.assertNotIn("getByPlaceholder('Search agents by name',{exact:true})", source)
        ...
        self.assertIn("await option.click()", source)
        self.assertIn("stage='agent_selection_confirm'", source)
        self.assertIn("selectedAgentName===name", source)
        self.assertIn("if(selectedAgentName!==name)throw Error('AGENT_SELECTION_MISMATCH')", source)
        self.assertIn("AGENT_SELECTION_MISMATCH", source)
        self.assertIn("stage='agent_builder_back'", source)
        self.assertIn("getByRole('button',{name:'Back to builder',exact:true}).click()", source)
        self.assertNotIn("search.press('Enter')", source)
        ...
        self.assertIn("await form.getByRole('button',{name:'Select Agent',exact:true}).click()", source)
        ...
        self.assertIn("const form=await openAgentBuilder(page,value=>{stage=value})", source)
        self.assertNotIn("agent_selection_retry", source)
